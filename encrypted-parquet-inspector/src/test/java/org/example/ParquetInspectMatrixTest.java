...
public class ParquetInspectMatrixTest {
  ...
  private static final List<String> COLS = List.of("name", "age", "salary",
      "ssn");
  private static final List<String> PARTIAL_FOOTER_KEY_COLS = List.of("name",
      "age");

  @BeforeAll
  static void checkOutDir() {
    assertTrue(new File(OUT_DIR).exists(), "Run ParquetGenerateAll first to create ./out files");
  }
  ...
  void matrix(String algo, String mode, String aad, String layout, String keymeta, int keySize, String fileName)
    ...
    cmd.add("org.example.ParquetInspect");
    cmd.add(f.getPath());
    cmd.add(USER_KEY);
